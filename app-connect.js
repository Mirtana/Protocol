...
document.getElementById('mintAmountEth').addEventListener('input', function() {
    ...
    if (val > 0 && currentPriceWei > 0n) {
        ...
        // Количество токенов = всего оплачено / цена за один
...
async function mintToken() {
    ...
    try {
        ...
        // 1. Считаем количество ЦЕЛЫХ токенов (например, 1000)
        // Делим общую сумму в Wei на цену 1 токена в Wei
        const tokensCount = weiToPay / currentPriceWei; 

        // 2. ВАЖНО: Твой контракт сам делает * 10**18 внутри.
        // Поэтому мы передаем просто чистое число tokensCount.
        const amountForContract = tokensCount; 

        openModal('loading', 'Waiting for wallet confirmation...');

        // Отправляем транзакцию
        const tx = await MIRTAContract.mint(amountForContract, { value: weiToPay });
        ...
        // Если ошибка повторится, посмотри в консоль на значение amountForContract
...
function openModal(type, message, txHash = null) {
    ...
    const closeBtn = document.getElementById('modalCloseBtn'); // Находим нашу кнопку
    ...
    if (type === 'loading') {
        ...
        closeBtn.style.display = 'none'; // СКРЫВАЕМ кнопку, пока идет загрузка
    } 
    else if (type === 'success') {
        title.innerText = "Success!";
        icon.innerHTML = '<i class="fas fa-check-circle" style="color: #00ff88;"></i>';
        desc.innerText = message;
        closeBtn.style.display = 'block'; // ПОКАЗЫВАЕМ кнопку, когда всё готово
    } 
    else if (type === 'error') {
        title.innerText = "Error";
        icon.innerHTML = '<i class="fas fa-times-circle" style="color: #ff4d4d;"></i>';
        desc.innerText = message;
        closeBtn.style.display = 'block'; // ПОКАЗЫВАЕМ кнопку, чтобы юзер мог закрыть ошибку
    }
