...
document.getElementById('mintAmountEth').addEventListener('input', function() {
    ...
    if (val > 0 && currentPriceWei > 0n) {
        ...
        document.getElementById('calcOutput').innerText = `${amount.toString()} MCF`;
    } else {
    ...
    try {
        ...
        const wasAdded = await window.ethereum.request({
            ...
            params: {
                type: 'ERC20', 
                options: {
                    address: config.MIRTA, // Адрес контракта MCF
                    symbol: 'MCF',        // Символ токена
        ...
        if (wasAdded) {
            console.log('Токен MCF добавлен в кошелек!');
        }
    } catch (error) {
